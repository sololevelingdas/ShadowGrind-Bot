...
async def main_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "in_onboarding":
        ...
        if code_data.get("used_by"):
            await processing_msg.edit_text("\U0001f6ab Code Already Bound.") # Prohibited 🚫
            user_ref.update({"state": firestore.DELETE_FIELD})
            return
        ...
        if not expires_at or not isinstance(expires_at, datetime) or datetime.now(timezone.utc) > expires_at:
            await processing_msg.edit_text("\u23f3 Code Expired.") # Hourglass ⏳
            user_ref.update({"state": firestore.DELETE_FIELD})
            return

        await processing_msg.edit_text("\u26d3\ufe0f Binding Contract...") # Chains ⛓️
        await asyncio.sleep(1.5)
        ...
        if is_returning_user:
            ...
            await asyncio.sleep(1.0)
            user_ref.update({
                "expires_at": expires_at, "package": "Abyss Protocol - Renewed",
                "state": firestore.DELETE_FIELD # Clear state
            })
            code_ref.update({"used_by": user_id, "activated_by_username": username, "renewal": True})
            new_days_left = (expires_at - datetime.now(timezone.utc)).days
            renewal_message = (
                f"**CONTRACT RENEWED // ACCESS EXTENDED**\n\n"
                f"Welcome back, Hunter @{username}.\n\n"
                f"Access extended. Contract valid for `{new_days_left}` more days.\n\n"
                f"Continue your grind. Use the interface below."
            )
            # Show reply keyboard after successful renewal
            await processing_msg.edit_text(renewal_message, parse_mode=ParseMode.MARKDOWN, reply_markup=MAIN_REPLY_MARKUP)

        else: # New user activation
            # (Set user data as before)
            user_ref.set({
                "real_name": user_data.get("real_name", "N/A"), "player_name": user_data.get("player_name", username),
                "dob": user_data.get("dob", "N/A"), "weight_initial": user_data.get("weight", "N/A"),
                "primary_aim": user_data.get("primary_aim", "N/A"), "username": username,
                "package": "Abyss Protocol", "activated_at": firestore.SERVER_TIMESTAMP,
                "expires_at": expires_at, "xp": 0, "level": 1, "rank": "E",
                "inventory": {}, "perks": [], "current_mission": None,
                "state": firestore.DELETE_FIELD, "has_accepted_honor_code": False
            }, merge=True)
            code_ref.update({"used_by": user_id, "activated_by_username": username, "renewal": False})

            welcome_caption = (f"**SYSTEM ONLINE // CONTRACT SEALED**\n\n"
                               f"Welcome, Hunter @{username}.\n\n"
                               f"Acknowledge the code...")
            try: # Try sending photo + honor code
            ...
            except Exception as e: # Fallback if photo fails
                print(f"Error sending welcome photo/honor code: {e}")
                try: await processing_msg.delete()
                except: pass
                await update.message.reply_text(welcome_caption, parse_mode=ParseMode.MARKDOWN)
                try: await update.message.reply_text(text=HONOR_CODE_TEXT, reply_markup=HONOR_KEYBOARD, parse_mode=ParseMode.MARKDOWN)
                except Exception as inner_e:
                     print(f"Fallback Honor Code failed: {inner_e}")
                     await update.message.reply_text("System Error during activation. Code: HC_FALLBACK")
